...
// const baseURL = "http://192.168.0.179:8003/api/auth";  // Base URL for the API
...
//       toast.success('Login successful');
//       // Store the token in localStorage (or cookies, etc.)
...
//     try {
//       const response = await axios.post(`${baseURL}/validate-otp`, { email, otp });
...
export function LoginForm({ className, ...props }) {
  ...
  // Handle Reset Password Flow (same as before)
  const handleResetPassword = async () => {
    try {
      const response = await axios.post(`${baseURL}/forgot-password`, { email });
      setIsOtpSent(true); 
  ...
  // Handle Password Reset and Submission
  const handleSubmitNewPassword = async () => {
    if (newPassword === confirmPassword) {
      try {
        await axios.post(`${baseURL}/reset-password`, { email, otp, newPassword });
